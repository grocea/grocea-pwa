...
function describeSyncIssue(issue: { type: string; error?: { code: string; message: string } }) {
  ...
  if (issue.error?.code === 'STATE_REVISION_CONFLICT') return { title: 'Account changed elsewhere', message: 'Another device changed this account first. Review the latest server state, then retry this local change deliberately or discard it.' }
