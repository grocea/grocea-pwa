...
export function CategoriesScreen() {
  ...
  return <AppShell navigation><BrandHeader /><main className="screen-content"><PageHeading title="Categories" subtitle={`${categories.filter(item => item.scope === 'global').length} global · ${categories.filter(item => item.scope === 'custom').length} yours`} action={<button type="button" className="button primary compact" onClick={() => { setAdding(true); setSubmitted(false) }}><Plus />New</button>} />{adding && <form className="inline-create" onSubmit={submit} noValidate aria-busy={createAction.pending}><label><span className="sr-only">Category name</span><input ref={nameRef} autoFocus value={name} disabled={createAction.pending} aria-invalid={submitted && Boolean(error)} aria-describedby="category-name-error" onChange={event => setName(event.target.value)} placeholder="Category name" /></label><button className="button primary" disabled={createAction.pending}>{createAction.pending ? 'Adding…' : 'Add'}</button><button className="button secondary" disabled={createAction.pending} type="button" onClick={() => { setAdding(false); setName(''); setSubmitted(false) }}>Cancel</button>{submitted && error && <span className="field-error inline-create-error" id="category-name-error" role="alert">{error}</span>}</form>}<label className="search-field"><MagnifyingGlass /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search categories" /><span className="sr-only">Search categories</span></label>{(['global', 'custom'] as const).map(scope => { const scoped = shown.filter(item => item.scope === scope); return <section className="category-section" key={scope}><div className="section-label"><strong>{scope === 'global' ? 'GLOBAL' : 'YOURS'}</strong><span>{scope === 'global' ? 'READ-ONLY' : 'CUSTOM'}</span></div>{scoped.length ? <div className="data-list">{scoped.map(category => <div className="category-row" key={category.id}><span className="menu-icon"><FolderSimple /></span><span><strong>{category.name}</strong><small>{ingredients.filter(item => item.categoryId === category.id).length} ingredients</small></span>{scope === 'global' ? <span className="scope-mark" role="img" aria-label="Global category" title="Global category"><LockSimple aria-hidden="true" /></span> : <OwnershipMark label="Your category" />}</div>)}</div> : query.trim() ? <EmptyState icon={FolderSimple} title={`No ${scope === 'global' ? 'global' : 'custom'} categories found`} message="Try another search or clear the filter." action={<button className="button secondary" type="button" onClick={() => setQuery('')}>Clear search</button>} /> : null}</section> })}<div className="info-banner"><Info /><span>To delete a category, first assign its ingredients to another category.</span></div></main></AppShell>
}

function describeSyncIssue(issue: { type: string; error?: { code: string; message: string } }) {
  if (issue.error?.code === 'LOCAL_ID_UNMAPPED') return { title: 'Local change needs remapping', message: 'This local change refers to data that is not linked to your account. Discard the change. After the first sync, create it again.' }
  if (issue.error?.code === 'STATE_REVISION_CONFLICT') return { title: 'Account changed elsewhere', message: 'Another device changed this account. Review the server data. Then retry or discard this local change.' }
  if (issue.error?.code === 'GROCERY_CALCULATION_STALE') return { title: 'Grocery plan needs review', message: 'Recipe or pantry data changed during sync. Review the restored Basket. Then create the list again.' }
  if (issue.error?.code === 'DEPENDENCY_FAILED') return { title: 'A related change did not sync', message: 'An earlier change did not sync. Review that issue, then retry sync.' }
  if (issue.error?.code === 'NETWORK_UNAVAILABLE') return { title: 'Waiting for connection', message: 'Grocea saved this change on your device. Grocea will retry when the service is available.' }
  if (issue.type === 'grocery-list.create') return { title: 'Grocery list could not sync', message: 'Review your Basket, then create the grocery list again.' }
  if (issue.type.startsWith('stock.')) return { title: 'Pantry change needs attention', message: 'The server does not have this pantry change. Retry sync to send it again.' }
  if (issue.type.startsWith('recipe.')) return { title: 'Recipe change needs attention', message: 'The recipe is available on this device. Retry sync to send the change again.' }
  return { title: 'Change needs attention', message: 'This change is available on this device. Retry sync, or discard the change if you do not need it.' }
}

function initialSyncCause(syncError: SyncError | null) {
  if (syncError?.code === 'NETWORK_UNAVAILABLE' || syncError?.status === 0) return 'The Grocea service is unavailable. You can view starter data, but you cannot edit it until the first sync is complete.'
  if (syncError && syncError.status >= 500) return 'The Grocea service has a problem. You can view starter data, but you cannot edit it until the first sync is complete.'
  return 'This account did not complete its first sync. You can view starter data. Grocea replaces it with your account data after the first sync.'
}
...
function syncStatusNotice(syncStatus: SyncStatus, pendingMutationCount: number, hasFailures: boolean, hasConflicts: boolean, syncError: SyncError | null, onRetry: () => void, retrying: boolean) {
  if (hasConflicts) return <div className="warning-banner"><WarningCircle /><span><strong>Imported data needs review</strong><small>Resolve the conflicts below to complete synchronization.</small></span></div>
  if (hasFailures) return <div className="warning-banner danger"><WarningCircle /><span><strong>{pendingMutationCount} change{pendingMutationCount === 1 ? '' : 's'} need attention</strong><small>Local changes are available. Retry or discard each change.</small></span></div>
  ...
  if (syncStatus === 'offline') return <div className="info-banner"><WifiSlash /><span><strong>Offline</strong><br />{pendingMutationCount ? `${pendingMutationCount} change${pendingMutationCount === 1 ? ' is' : 's are'} saved on this device and queued for sync.` : 'Grocea saves changes on this device until the service is available.'}</span></div>
  if (syncStatus === 'syncing') return <div className="info-banner"><ArrowClockwise /><span><strong>Syncing changes</strong><br />You can continue to work while Grocea syncs local changes.</span></div>
  ...
  return <div className="success-notice"><CheckCircle />The local cache and Grocea service are up to date.</div>
}
...
export function ProfileScreen() {
  ...
  const passwordAction = usePendingAction(async () => {
    ...
    setNewPassword('')
    setRepeatPassword('')
    setPasswordMessage('Grocea changed your password and signed out other sessions.')
  })
  async function submit(event: FormEvent) {
    event.preventDefault()
    if (!name.trim()) return
