...
function Stepper({ value, label, disabled, onChange }: { value: number; label: string; disabled: boolean; onChange: (value: number) => void }) {
  return <div className="stepper"><button type="button" disabled={disabled || value <= 1} aria-label={`Decrease servings for ${label}`} onClick={() => onChange(Math.max(1, value - 1))}>−</button><strong>{value}</strong><button type="button" disabled={disabled || value >= 12} aria-label={`Increase servings for ${label}`} onClick={() => onChange(Math.min(12, value + 1))}>+</button></div>
}
...
export function BasketScreen() {
  ...
  return <AppShell><BackHeader title="Basket" fallbackTo="/recipes" eyebrow={`${basket.length} recipe${basket.length === 1 ? '' : 's'}`} />
    <main className="detail-screen grocery-screen">
      <PageHeading title="Plan your groceries" subtitle="Adjust servings before Grocea calculates what you need to buy." />
      ...
      {!basket.length ? <EmptyState icon={Basket} title="Basket is empty" message="Add published Recipes, then return here to review servings." action={<Link className="button primary" to="/recipes">Browse recipes</Link>} /> : <section className="basket-list" aria-label="Selected recipes">{basket.map(item => <article className={`basket-row${item.valid ? '' : ' invalid'}`} key={item.recipeId}><span className="recipe-art"><Receipt /></span><span><strong>{item.recipeName}</strong><small>{item.valid ? `Base serves ${item.baseServings}` : item.error}</small></span><Stepper value={item.servings} label={item.recipeName} disabled={confirmAction.pending || !item.valid} onChange={servings => void addRecipeToBasket(item.recipeId, servings)} /><button className="icon-button danger-text" type="button" aria-label={`Remove ${item.recipeName} from Basket`} disabled={confirmAction.pending} onClick={() => void removeRecipeFromBasket(item.recipeId)}><Trash /></button></article>)}</section>}
      {basket.length > 0 && <button className="text-button danger-text" type="button" onClick={() => setClearOpen(true)}>Clear Basket</button>}
    </main>
    {basket.length > 0 && <div className="form-actions sticky"><Link className="button secondary" to="/recipes">Add more recipes</Link><button className="button primary" type="button" disabled={Boolean(active) || Boolean(staleCalculation) || invalid || confirmAction.pending} aria-busy={confirmAction.pending} onClick={() => void confirmAction.run()}><ListChecks />{confirmAction.pending ? 'Calculating…' : 'Create Grocery List'}</button></div>}
...
export function GroceryListScreen() {
  ...
  return <AppShell><BackHeader title={active ? 'Active Grocery List' : 'Completed Grocery List'} fallbackTo="/groceries" eyebrow={active ? `${list.items.filter(item => item.checked).length}/${list.items.length} checked` : new Date(list.completedAt ?? list.createdAt).toLocaleDateString()} />
    <main className="detail-screen grocery-screen"><SuccessNotice message={message} />{error && <div className="warning-banner danger" role="alert"><WarningCircle /><span>{error}</span></div>}
      {active ? <form className="grocery-title-form" onSubmit={event => { event.preventDefault(); void renameGroceryList(list.id, title) }}><label><span className="sr-only">Grocery List title</span><input value={title} maxLength={120} onChange={event => setTitle(event.target.value)} /></label><button className="button secondary compact" disabled={!title.trim() || title.trim() === list.title}><Check />Save title</button></form> : <section className="hero-card compact"><span className="eyebrow">COMPLETED</span><h1>{list.title}</h1><p>{list.recipes.length} source recipe{list.recipes.length === 1 ? '' : 's'} · {list.items.length} grocery item{list.items.length === 1 ? '' : 's'}</p></section>}
      {active && <button className="button secondary" type="button" onClick={() => setEditing('new')}><Plus />Add grocery item</button>}
      ...
      {!list.items.length ? <EmptyState icon={CheckCircle} title="Nothing to buy" message="Your Pantry already covered every calculated Ingredient." /> : groups.map(group => <section className="grocery-group" key={group.category}><div className="section-label"><strong>{group.category}</strong><span>{group.items.length}</span></div><div className="grocery-items">{group.items.map(item => <article className={`grocery-item${item.checked ? ' checked' : ''}`} key={item.id}>{active ? <label className="grocery-check"><input type="checkbox" checked={item.checked} aria-label={`Mark ${item.label} ${item.checked ? 'not purchased' : 'purchased'}`} onChange={() => void updateGroceryItem(list.id, { ...item, checked: !item.checked })} /></label> : <span className="completion-mark"><Check /></span>}<span className="grocery-item-copy"><strong>{item.label}{item.edited && <small className="edited-badge">Edited</small>}</strong><small>{amountLabel(item)}</small>{item.sources.length > 0 && <details><summary>Recipe breakdown</summary><div>{item.sources.map(source => <span key={source.recipeId}>{source.recipeName} · {formatQuantity(source.quantity, item.family ?? 'count')} for {source.servings} serving{source.servings === 1 ? '' : 's'}</span>)}{item.originalPantry !== undefined && <span>Pantry at creation · {formatQuantity(item.originalPantry, item.family ?? 'count')}</span>}</div></details>}</span>{active && <span className="grocery-item-actions"><button className="icon-button" type="button" aria-label={`Edit ${item.label}`} onClick={() => setEditing(item)}><PencilSimple /></button><button className="icon-button danger-text" type="button" aria-label={`Delete ${item.label}`} onClick={() => void removeGroceryItem(list.id, item.id)}><Trash /></button></span>}</article>)}</div></section>)}
      <section className="source-recipes"><div className="section-label"><strong>Source recipes</strong><span>{list.recipes.length}</span></div>{list.recipes.map(recipe => <div key={recipe.recipeId}><span>{recipe.recipeName}</span><small>{recipe.servings} servings</small></div>)}</section>
      {active ? <div className="destructive-actions"><button className="text-button" type="button" onClick={() => setDeleteMode('restore')}>Restore recipes to Basket and delete</button><button className="text-button danger-text" type="button" onClick={() => setDeleteMode('delete')}>Delete list</button></div> : <div className="inline-actions"><button className="button secondary" type="button" onClick={() => void reuseGroceryList(list.id).then(() => navigate('/recipes/basket')).catch(cause => setError(cause instanceof Error ? cause.message : 'Recipes could not be reused.'))}><Basket />Reuse recipes</button><button className="button danger" type="button" onClick={() => setDeleteMode('delete')}><Trash />Delete history</button></div>}
    </main>
    {active && <div className="form-actions sticky"><Link className="button secondary" to="/groceries">Back to Groceries</Link><button className="button primary" type="button" onClick={openCompletion}><CheckCircle />Complete list</button></div>}
    <dialog ref={completionRef} className="pantry-completion-dialog" aria-labelledby="complete-list-title" onCancel={event => { event.preventDefault(); if (!completeAction.pending) setCompletionOpen(false) }}><section className="pantry-completion"><h2 id="complete-list-title">Complete Grocery List?</h2>{list.items.some(item => !item.checked) && <div className="warning-banner"><WarningCircle /><span><strong>{list.items.filter(item => !item.checked).length} unchecked item{list.items.filter(item => !item.checked).length === 1 ? '' : 's'}</strong><small>List can still be completed.</small></span></div>}<p>Choose purchased catalog items to add to Pantry.</p>{eligible.length ? <div className="pantry-preview">{eligible.map(item => <label key={item.id}><input type="checkbox" checked={selectedPantryIds.has(item.id)} onChange={() => setSelectedPantryIds(current => { const next = new Set(current); if (next.has(item.id)) next.delete(item.id); else next.add(item.id); return next })} /><span><strong>{item.label}</strong><small>{amountLabel(item)} · Pantry becomes {formatQuantity((balances[item.ingredientId!] ?? 0n) + (pantryPreviewTotals.get(item.ingredientId!) ?? 0n), item.family!)}</small></span></label>)}</div> : <p className="muted-copy">No checked catalog items with amounts are eligible for Pantry update.</p>}<div className="inline-actions"><button ref={completionCancelRef} className="button secondary" disabled={completeAction.pending} onClick={() => setCompletionOpen(false)}>Cancel</button><button className="button primary" disabled={completeAction.pending} onClick={() => void completeAction.run().catch(cause => setError(cause instanceof Error ? cause.message : 'List could not be completed.'))}>{completeAction.pending ? 'Completing…' : selectedPantryIds.size ? 'Update Pantry & complete' : 'Complete without Pantry update'}</button></div></section></dialog>
