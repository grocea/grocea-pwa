...
export function RecipeListScreen() {
  ...
  return <AppShell navigation>
    ...
    <main className="screen-content recipe-list-screen">
      ...
      {tab === 'drafts' ? <section className="recipe-grid" aria-labelledby="recipe-results-title">
        ...
        {!shownPublished.length && <EmptyState
          ...
          message={tab === 'ready' && !query.trim() ? 'No recipe has all required ingredients in stock. View all recipes to cook with missing ingredients.' : 'Try another search or clear your filter.'}
          action={query.trim()
    ...
    <ConfirmDialog open={Boolean(deleteTarget)} title="Delete draft?" description={`This permanently deletes ${deleteTarget?.name ?? 'this draft'} and its draft details.`} confirmLabel="Delete draft" pendingLabel="Deleting…" onDismiss={() => setDeleteTarget(null)} onConfirm={async () => { if (deleteTarget) await deleteRecipeDraft(deleteTarget.id) }} />
  </AppShell>
}
...
export function CookPreviewScreen() {
  ...
  if (!recipe) return <AppShell><BackHeader title="Cook preview" fallbackTo="/recipes" /><EmptyState title="Recipe unavailable" message="You can cook only published recipes." /></AppShell>
  const short = changes.filter(change => change.after < 0n)
  return <AppShell><BackHeader title="Cook preview" fallbackTo={`/recipes/${recipe.id}`} eyebrow="Review pantry changes" /><main className="detail-screen"><section className="hero-card compact"><span className="eyebrow">READY TO COOK</span><h1>{recipe.name}</h1><p>{recipe.description}</p></section><div className="stepper-row card"><span><strong>Servings</strong><small>Grocea updates amounts when you change servings</small></span><Stepper value={servings} onChange={setServings} disabled={cookAction.pending} /></div>{short.length > 0 && <div className="warning-banner"><WarningCircle /><span><strong>Cooking creates {short.length} negative balance{short.length > 1 ? 's' : ''}</strong><small>This moves those ingredients to Needs restock.</small></span></div>}
